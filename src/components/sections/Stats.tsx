      ...
      ([entry]) => {
        if (entry.isIntersecting && !hasAnimated) {
          ...
          const animate = (timestamp: number) => {
            ...
            // Use easeOutCubic for smooth deceleration and prevent glitch
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 lg:gap-12">
          {stats.map((stat, index) => (
            <div
              key={stat.label}
              className="text-center p-5 sm:p-7 rounded-2xl glass gradient-border hover-lift animate-fade-in-up relative overflow-hidden group"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className={`absolute -inset-px opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-2xl pointer-events-none ${
                stat.color === "cyan" ? "bg-cyan/20" :
                stat.color === "magenta" ? "bg-magenta/20" :
                stat.color === "purple" ? "bg-purple/20" :
                "bg-orange/20"
              }`} />

              <div className="relative z-10">
                <AnimatedCounter end={stat.value} suffix={stat.suffix} color={stat.color} />
                <p className="text-xs sm:text-sm md:text-base text-muted-foreground mt-2 font-medium uppercase tracking-wider">{stat.label}</p>
              </div>
            </div>
          ))}
        </div>
