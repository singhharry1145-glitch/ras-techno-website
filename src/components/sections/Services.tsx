          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {services?.map((service, index) => {
              ...
              return (
                ...
                >
                <div
                  className="group relative p-6 sm:p-7 rounded-2xl glass gradient-border hover-lift shimmer flex flex-col overflow-hidden h-full"
                >
                  <div className={`absolute -inset-1 opacity-0 group-hover:opacity-100 transition-opacity duration-700 blur-2xl pointer-events-none ${
                    service.color === "cyan" ? "bg-cyan/20" :
                    service.color === "magenta" ? "bg-magenta/20" :
                    service.color === "purple" ? "bg-purple/20" : "bg-orange/20"
                  }`} />
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    <div
                      className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 ${
                        service.color === "cyan"
                          ? "bg-gradient-to-br from-cyan/30 to-cyan/5 text-cyan shadow-lg shadow-cyan/20"
                          : service.color === "magenta"
                          ? "bg-gradient-to-br from-magenta/30 to-magenta/5 text-magenta shadow-lg shadow-magenta/20"
                          : service.color === "purple"
                          ? "bg-gradient-to-br from-purple/30 to-purple/5 text-purple shadow-lg shadow-purple/20"
                          : "bg-gradient-to-br from-orange/30 to-orange/5 text-orange shadow-lg shadow-orange/20"
                      }`}
                    >
                      <IconComponent className="w-7 h-7" />
                    </div>

                    <h3 className="font-display text-xl font-bold text-foreground mb-3 group-hover:text-gradient-primary transition-all">{service.title}</h3>
                    <p className="text-sm sm:text-base text-muted-foreground mb-5 flex-grow leading-relaxed">{service.description}</p>
                    ...
                    <ul className="space-y-2 mb-5">
                      {service.features.map((feature) => (
                        <li key={feature} className="text-xs sm:text-sm text-muted-foreground flex items-center gap-2">
                          ...
                          } shadow-sm`} style={{ boxShadow: `0 0 8px hsl(var(--${service.color}))` }} />
                          {feature}
                        </li>
                      ))}
                    </ul>

                    <Button variant="ghost" className="w-full group/btn mt-auto" onClick={() => scrollTo('solutions')}>
                      Learn more
